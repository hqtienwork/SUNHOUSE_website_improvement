      ...
      <table class="table table-striped b-t b-light">
        ...
        <tr>
          <td colspan="2">  
            @php 
                $total_coupon = 0;
                $feeship = $order_details->first()->product_feeship ?? 0;
            @endphp
            @if($coupon_condition==1)
                @php
                $total_after_coupon = ($total * $coupon_number) / 100;
                echo 'Tổng giảm :'.number_format($total_after_coupon,0,',','.').'</br>';
                $total_coupon = $total + $feeship - $total_after_coupon;
                @endphp
            @else 
                @php
                echo 'Tổng giảm :'.number_format($coupon_number,0,',','.').'k'.'</br>';
                $total_coupon = $total + $feeship - $coupon_number;
                @endphp
            @endif

            Phí ship : {{number_format($feeship,0,',','.')}}đ</br> 
          ...
          @php
            $feeship = $order_details->first()->product_feeship ?? 0;
            $order_code = $order_details->first()->order_code ?? 'unknown';
          @endphp
